...
def get_colleges_with_partial_weather(conn, limit=5):
    ...
    cur.execute(
        """
        SELECT c.id, c.name, c.lat, c.lon, COUNT(w.id) AS weather_count
        FROM colleges c
...
def main():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT COUNT(*) FROM colleges WHERE lat IS NOT NULL AND lon IS NOT NULL"
    )
    ...
    if not colleges:
        ...
        if colleges_partial:
            ...
            colleges = [(c[0], c[1], c[2], c[3]) for c in colleges_partial]
        else:
    ...
    for college_id, name, lat, lon in colleges:
        ...
        if not weather_data:
            print(" Warning: No weather data received, skipping...")
            continue
